...
class JobExecutor
{
    ...
    public static function executor($job_group = '')
    {
         ...
         */
        $task = JobTask::where('task_status', 0)
        ->where('sched_time', '<=', date('Y-m-d H:i:s'))
        ->where('is_cancel', 0)
        ...
        if(!empty($task)){ //endif 条件查询task，第一次触发的优先执行
            ...
            $task = JobTask::where('task_status', 4)
            ->where('sched_time', '<=', date('Y-m-d H:i:s'))
            ->where('is_cancel', 0)
    ...
    private static function transTask($task)
    {
        try{
            ...
            //请求排它锁
            $task_u = JobTask::where('id', $task['id'])->whereIn('task_status', [0,4])->where('is_cancel', 0)->lockForUpdate()->first();
...
//                             $job['exec_data'] = 'http://www.baidu.com';
                    ...
                    JobTask::where('id', $task['id'])->whereIn('task_status', [0,4])->where('is_cancel', 0)->update($update);
